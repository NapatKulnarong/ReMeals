...
class RestaurantChainAPITests(APITestCase):
    ...
    def test_create_chain(self):
        payload = {"chain_id": "CHA10", "chain_name": "New Chain"}
        res = self.client.post("/api/restaurant-chains/", payload, format="json")
        self.assertEqual(res.status_code, status.HTTP_201_CREATED)
        self.assertTrue(RestaurantChain.objects.filter(chain_id="CHA10").exists())

    # 2. List restaurant chains
    def test_list_chains(self):
        RestaurantChain.objects.create(chain_id="CHA11", chain_name="Chain 11")
    ...
    def test_delete_chain(self):
        chain = RestaurantChain.objects.create(chain_id="CHA12", chain_name="Chain 12")
        res = self.client.delete(f"/api/restaurant-chains/{chain.chain_id}/")
        self.assertEqual(res.status_code, status.HTTP_204_NO_CONTENT)
        self.assertFalse(RestaurantChain.objects.filter(chain_id="CHA12").exists())

    # 4. Create chain requires chain_name
    def test_create_chain_requires_name(self):
        payload = {"chain_id": "CHA13"}
        res = self.client.post("/api/restaurant-chains/", payload, format="json")
        self.assertEqual(res.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertFalse(RestaurantChain.objects.filter(chain_id="CHA13").exists())

    # 5. Updating chain name via PUT works
    def test_update_chain_name(self):
        chain = RestaurantChain.objects.create(chain_id="CHA14", chain_name="Old Name")
        payload = {"chain_id": "CHA14", "chain_name": "Updated Name"}
    ...
    def test_patch_chain_name(self):
        chain = RestaurantChain.objects.create(chain_id="CHA15", chain_name="Patch Name")
    ...
    def test_get_chain_detail(self):
        chain = RestaurantChain.objects.create(chain_id="CHA16", chain_name="Detail Chain")
    ...
    def test_create_duplicate_chain_id_fails(self):
        RestaurantChain.objects.create(chain_id="CHA17", chain_name="First")
        payload = {"chain_id": "CHA17", "chain_name": "Duplicate"}
        res = self.client.post("/api/restaurant-chains/", payload, format="json")
        self.assertEqual(res.status_code, status.HTTP_400_BAD_REQUEST)
